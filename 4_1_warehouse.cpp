...
vector<vector<vector<long long>>> dp_trip(2, vector<vector<long long>>(1 << W,
...
dp_trip[0][new_mask][next] = min(dp_trip[0][new_mask][next],
dp_trip[0][mask][last] + segment_cost);
}
if (dp_trip[1][mask][last] != INF) {
dp_trip[1][new_mask][next] = min(dp_trip[1][new_mask][next],
dp_trip[1][mask][last] + segment_cost);
}
}
}
}
